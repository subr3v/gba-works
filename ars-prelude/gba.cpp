...
{
    // Optimized version.
    volatile uint32_t* objBuffer = (volatile uint32_t*)REG_OBJ_BASE;
    volatile uint32_t* objBufferEnd = (volatile uint32_t*)REG_OBJ_BASE + sizeof(ObjBuffer) / sizeof(uint32_t);
    uint32_t* srcBuffer = (uint32_t*)ObjBuffer;
    while (objBuffer != objBufferEnd)
    {
        *objBuffer++ = *srcBuffer++;
        *objBuffer++ = *srcBuffer++;
    }
